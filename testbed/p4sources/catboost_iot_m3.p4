...
control InferenceModel(
    ...
    action SetCode5(bit<12> code0) {
        codewords.codeword0_5 = code0;
    }
    action SetCode6(bit<3> code0) {
        codewords.codeword0_6 = code0;
    }
    action SetCode7(bit<1> code0) {
        codewords.codeword0_7 = code0;
    }
    action SetCode8(bit<13> code0) {
        codewords.codeword0_8 = code0;
    }
    action nop() {}

    Features_t features;
    // FEATURES: ['dstport', 'srcport', 'tcp.window_size_value', 'ip.ttl', 'tcp.hdr_len', 'ip.len', 'udp.length', 'tcp.flags.ack', 'Max Packet Length']
    table TableFeature0 {
    ...
    table TableFeature3 {
        key = { hdr.ipv4.ttl: range @name("feature3"); }
