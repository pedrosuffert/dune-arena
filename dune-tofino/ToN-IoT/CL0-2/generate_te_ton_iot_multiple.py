...
clf_F = pd.read_pickle('/home/michele/distributed_in_band/INFOCOM2025/ToN-IoT/SPP/4CLUSTER/CL0-2/ToN_IoT_ClID2_T1_F6_L85_N3_With_Others.sav')
RF_models.append(clf_F)
clf_S = pd.read_pickle('/home/michele/distributed_in_band/INFOCOM2025/ToN-IoT/SPP/4CLUSTER/CL0-2/ToN_IoT_ClID0_T3_F5_L41_N4_With_Others.sav')
RF_models.append(clf_S)
## list the feature names
feature_names_F = RF_models[0].feature_names_in_
print(feature_names_F)
#
feature_names_S = RF_models[1].feature_names_in_
...
def get_feature_table(splits_data, feature_name):
    ...
    print('Feature Name: ', feature_name, 'Threshold: ', list(feature_data["Threshold"].unique()))
    ##
    ...
    # print('Threshold: ', code_table["Threshold"])
...
with open("te_ton_jewel_1_2_85_N2_1_6_85_N4.py", "w") as entries_file:

    print("from netaddr import IPAddress\n", file=entries_file)

    print("p4 = bfrt.ton_jewel_1_2_85_N2_1_6_85_N4.pipe\n", file=entries_file)

    # print("exec(open('/home/akem/p4-code/CICIDS_pForest_Tofino/include/clear_tables.py').read())", file=entries_file)
    clear_tables = """
...
###################################### GENERATE FEATURE and CODE TABLES for MULTIPLE MODELS ###############################################
    feat_index = 0
    code_table_index = 0
    count = 0
    print('------')
    for clf in RF_models:
        ...
        for num_feat in range(len(feature_names)):
                print("table_feature"+str(num_feat+feat_index)+" = p4.Ingress.table_feature"+str(num_feat+feat_index), file=entries_file)
        print('', file=entries_file)

        for num_tree in range(len(clf.estimators_)):
            print("code_table"+str(num_tree+code_table_index)+" = p4.Ingress.code_table"+str(num_tree+code_table_index), file=entries_file)
        print('', file=entries_file)
        ...
        if (count == 0):
            for fea in range(0,len(feature_names)):
                if feature_names[fea] != 'PSH Flag Count':
                    ...
                    for ran, cods0 in zip(Ranges, Codes.iloc[:,0]):
                        if(ran == Ranges[len(Ranges)-1]):
                            print("table_feature"+str(fea+feat_index)+".add_with_SetCode"+str(fea+feat_index)+"(feature"+str(fea+feat_index)+"_start="+str(ran.split(",")[0])+ \
                            ", feature"+str(fea+feat_index)+"_end="+str(65535)+", code0="+str(cods0) + ")", file = entries_file)
                        else:
                            print("table_feature"+str(fea+feat_index)+".add_with_SetCode"+str(fea+feat_index)+"(feature"+str(fea+feat_index)+"_start="+str(ran.split(",")[0])+ \
                            ", feature"+str(fea+feat_index)+"_end="+str(ran.split(",")[1])+", code0="+str(cods0) + ")", file = entries_file)
            ...
            for fea in range(0,len(feature_names)):
                ...
                for ran, cods0, cods1, cods2 in zip(Ranges, Codes.iloc[:,0], Codes.iloc[:,1], Codes.iloc[:,2]):
                    if(ran == Ranges[len(Ranges)-1]):
                        print("table_feature"+str(fea+feat_index)+".add_with_SetCode"+str(fea+feat_index)+"(feature"+str(fea+feat_index)+"_start="+str(ran.split(",")[0])+ \
                        ", feature"+str(fea+feat_index)+"_end="+str(65535)+", code0="+str(cods0) + ", code1=" + str(cods1) + ", code2=" + str(cods2) + ")", file = entries_file)
                    else:
                        print("table_feature"+str(fea+feat_index)+".add_with_SetCode"+str(fea+feat_index)+"(feature"+str(fea+feat_index)+"_start="+str(ran.split(",")[0])+ \
                        ", feature"+str(fea+feat_index)+"_end="+str(ran.split(",")[1])+", code0="+str(cods0)+", code1="+str(cods1) + ", code2=" + str(cods2) + ")", file = entries_file)
                tree_code0.append(len(cods0)-2)
                tree_code1.append(len(cods1)-2)
                tree_code2.append(len(cods2)-2)
                # tree_code3.append(len(cods3)-2)
        ...
        for tree_id in range(0, len(clf.estimators_)):
            ...
            for cod, mas, cla, cer in zip(Final_Codes, Final_Masks, Classe, Certain):
                # print("code_table"+str(tree_id)+".add_with_SetClass"+str(tree_id)+"(codeword"+str(tree_id)+"=", cod, ", codeword"+str(tree_id)+"_mask=", mas, ", classe=",cla+1,")", file=entries_file)
                print("code_table"+str(tree_id+code_table_index)+".add_with_SetClass"+str(tree_id+code_table_index)+"(codeword"+str(tree_id+code_table_index)+"=", cod, ", codeword"+str(tree_id+code_table_index)+"_mask=", mas, ", classe=",cla+1, ")", file=entries_file)
            print('', file=entries_file)
        ...
        if (count == 1):
            ...
            for i in range(1,4):
                for j in range(1,4):
                    for k in range(1,4):
                        if ((i!=j) & (j!=k) & (i!=k)):
                            print("voting_table.add_with_set_final_class("+"class1="+str(i)+ \
                                ", class2="+str(j)+", class3="+str(k)+", class_result="+str(np.random.choice([i, j, k]))+")", file=entries_file)
                            # pass
                        else:
                            print("voting_table.add_with_set_final_class("+"class1="+str(i)+", class2="+str(j)+\
                                ", class3="+str(k)+", class_result="+str(mode([i, j, k]))+")", file=entries_file)
        # for i in range(1,6):
        #     for j in range(1,6):
        #         if ((i!=j)):
        #             print("voting_table.add_with_set_final_class("+"class0="+str(i)+ \
        #                     ", class1="+str(j)+", class_result="+str(np.random.choice([i, j]))+")", file=entries_file)
        #             # pass
        #         else:
        #             print("voting_table.add_with_set_final_class("+"class0="+str(i)+", class1="+str(j)+", class_result="+str(mode([i, j]))+")", file=entries_file)
        print(feat_index)
        feat_index = feat_index + len(feature_names)
        code_table_index = code_table_index + len(clf.estimators_)
        print('\n', file=entries_file)
...
######################################################### COMMON FOR BOTH MODELS ###########################################################################
    ...
    flow_id_info = pd.read_csv("/home/michele/EXTENSION/ToN-IoT/ToN_IoT_Test_Flow_PktCounts.csv")
