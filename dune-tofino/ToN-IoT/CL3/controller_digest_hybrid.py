...
registers = ['Ingress.reg_flow_ID','Ingress.reg_time_last_pkt','Ingress.reg_pkt_count', 'Ingress.reg_classified_flag', \
    'Ingress.reg_pkt_len_min', 'Ingress.reg_pkt_len_max', 'Ingress.reg_pkt_len_total', 'Ingress.reg_ack_flag_count', 'Ingress.reg_psh_flag_count']
# Register to test
...
while True:
    ...
    if digest_type == 1:
        ...
        keys_reg = {'Ingress.reg_flow_ID': [],'Ingress.reg_time_last_pkt': [],
                    ...
                    'Ingress.reg_pkt_len_min': [], 'Ingress.reg_pkt_len_max': [],
                    'Ingress.reg_pkt_len_total': [], 'Ingress.reg_ack_flag_count': [],
                    'Ingress.reg_psh_flag_count': []}
        ...
        for dd in data_list:
            ...
            is_store = data_dict['is_store']
            is_flow = str(data_dict['is_flow'])

            if is_store == 0:
                csv_row = source_addr + ',' + destin_addr + ',' + source_port + ',' + destin_port + ',' + protocol + ',' + str(55) + ',' + str(55)
