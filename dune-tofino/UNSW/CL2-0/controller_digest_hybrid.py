...
# List of registers
registers = ['Ingress.reg_flow_ID','Ingress.reg_time_last_pkt','Ingress.reg_pkt_count', 'Ingress.reg_classified_flag_model1', 'Ingress.reg_classified_flag_model2', 'Ingress.reg_flow_iat_max', 'Ingress.reg_flow_iat_min', 'Ingress.reg_pkt_len_max']
...
while True:
    try:
        digest = interface.digest_get(timeout=10000)
    except:
        f = open("x.txt", "a")
        f.write('---- \n')
        f.close()
    ...
    if digest_type == 1:
        ...
        keys_reg = {'Ingress.reg_flow_ID': [], 'Ingress.reg_time_last_pkt': [],
                    ...
                    'Ingress.reg_classified_flag_model2': [], 'Ingress.reg_flow_iat_max': [], 
        ...
        for dd in data_list:
            ...
            is_flow = data_dict['is_flow']
            

            if (data_dict['is_store'] == 1):
                if flow_packet_class == 19:
                    csv_row = source_addr + ',' + destin_addr + ',' + source_port + ',' + destin_port + ',' + protocol + ',' + pkt_count + ',' + str(is_flow) + ',' + str(32)
                else:
                    csv_row = source_addr + ',' + destin_addr + ',' + source_port + ',' + destin_port + ',' + protocol + ',' + pkt_count + ',' + str(is_flow) + ',' + str(flow_packet_class)

            # else:
            #     csv_row = source_addr + ',' + destin_addr + ',' + source_port + ',' + destin_port + ',' + protocol + ',' + pkt_count + ',' +  str(is_flow) + ',' + str(55)
